...
def algo_run_v3(game:GeneralSumGame, save_id=None) -> np.ndarray:
    n_players = game.n_players
    gbt_strategy = np.zeros((n_players, 2))
    #strategy = np.zeros((n_players, 2))
    
    i_th_two_player_game = game.two_player_approximation(0) # First Player Approximation
    feasibility_strategy, _, feasibility_info = feasibility_run(game)
    feasibility_strategy = feasibility_strategy.reshape(3, 2)
    joint_dist = np.concatenate((feasibility_strategy[1], feasibility_strategy[2]))
    mariginal_strategy = compute_marginal_pmf(joint_dist)
    feasibility_strategy[1], feasibility_strategy[2] = mariginal_strategy[0], mariginal_strategy[1]
    ...
    for p in range(n_players):
        ...
        for act in gbt_result[0]['Player '+str(p+1)]:
            temp.append(act[1])
        i_th_two_player_strategy = np.array(temp)
        gbt_strategy[p] = i_th_two_player_strategy

    # 紀錄Two-Player Game中，Player 2的Nash Equilibria
    if save_id is not None and game.n_players == 3:
        # temp = []
        # for act in gbt_result[0]['Player 2']:
        #     temp.append(act[1])
        # oppo_player_strategy = np.array(temp)
        # oppo_player_strategy_mariginal = compute_marginal_pmf(oppo_player_strategy)
        # # 取得player 2、3的strategy
        # gbt_strategy[1], gbt_strategy[2] = oppo_player_strategy_mariginal[0], oppo_player_strategy_mariginal[1]
        ...
        with open('./runs/'+save_id+'.csv', 'w', newline='') as csvfile:
            ...
            # 寫入每個i-th two player game的結果
            writer.writerow(['Player 1\'s Two-Player Game'])
            strategy = [gbt_strategy, feasibility_strategy, feasibility_strategy_woindcon]
            algo_str = ['Gamebit Result', 'Feasibility Result', 'Feasibility Result(without independent constraint)']
            for algo in range(3):
                writer.writerow([])
                ...
                if algo >= 1:
                    ...
                    player_strategy.append(joint_dist if algo == 1 else joint_dist_woindcon)
                    is_nash_woindcon = NashEquilibriumJudger.run_indifference(player_strategy, i_th_two_player_game.payoff_matrix)
                    is_nash_write_data = ['[' + ', '.join(str(round(x, 2)) for x in (joint_dist if algo == 1 else joint_dist_woindcon)) + ']', \
                    ...
                    # More Constraint Info
                    info = feasibility_info if algo == 1 else feasibility_woindcon_info
                    writer.writerow([])
                    writer.writerow(['', '1,0', '1,1', '2,0', '2,1', '2,2', '2,3'])
                    ...
                    writer.writerow(['', '1,0', '1,1', '2,0', '2,1', '3,0', '3,1'])
                    all_cons = ['u', 'v', 'r', 'indifference']
                    for cons in all_cons:    
                        constraint_data = [cons]
...
if __name__ == '__main__':
    ...
    payoff_matrix = np.zeros(shape=(2, 2, 2, 3), dtype=int)
    # Special Case 3
    # payoff_matrix[0,0,0] = np.array([4,10, 1])
    # payoff_matrix[1,0,0] = np.array([8, 6, 1])
    # payoff_matrix[0,1,0] = np.array([3, 4, 3])
    # payoff_matrix[1,1,0] = np.array([2, 2, 4])
    # payoff_matrix[0,0,1] = np.array([9, 2, 3])
    # payoff_matrix[1,0,1] = np.array([7, 8, 9])
    # payoff_matrix[0,1,1] = np.array([3, 4, 7])
    # payoff_matrix[1,1,1] = np.array([8, 6, 2])

    # # 3_player_game_same3_not_satified_constraint_but_mixed_nash
    # payoff_matrix[0,0,0] = np.array([5, 2, 2])
    # payoff_matrix[1,0,0] = np.array([2, 8, 8])
    # payoff_matrix[0,1,0] = np.array([8,10, 10])
    # payoff_matrix[1,1,0] = np.array([10,3, 3])
    # payoff_matrix[0,0,1] = np.array([3, 9, 9])
    # payoff_matrix[1,0,1] = np.array([6, 2, 2])
    # payoff_matrix[0,1,1] = np.array([4, 6, 6])
    # payoff_matrix[1,1,1] = np.array([4, 5, 5])

    # 3_player_game_same1
    # payoff_matrix[0,0,0] = np.array([3, 4, 4])
    # payoff_matrix[1,0,0] = np.array([6, 0, 0])
    # payoff_matrix[0,1,0] = np.array([8, 4, 4])
    # payoff_matrix[1,1,0] = np.array([0, 9, 9])
    # payoff_matrix[0,0,1] = np.array([8, 7, 7])
    # payoff_matrix[1,0,1] = np.array([0, 3, 3])
    # payoff_matrix[0,1,1] = np.array([7, 0, 0])
    # payoff_matrix[1,1,1] = np.array([5, 0, 0])

    # 3_player_game_same4
    # payoff_matrix[0,0,0] = np.array([2, 5, 5])
    # payoff_matrix[1,0,0] = np.array([5, 4, 4])
    # payoff_matrix[0,1,0] = np.array([8,10,10])
    # payoff_matrix[1,1,0] = np.array([8, 4, 4])
    # payoff_matrix[0,0,1] = np.array([9, 9, 9])
    # payoff_matrix[1,0,1] = np.array([1, 1, 1])
    # payoff_matrix[0,1,1] = np.array([5,10,10])
    # payoff_matrix[1,1,1] = np.array([1, 0, 0])

    # 3_player_game_same5
    payoff_matrix[0,0,0] = np.array([7, 5, 5])
    payoff_matrix[1,0,0] = np.array([5,10,10])
    payoff_matrix[0,1,0] = np.array([8, 7, 7])
    payoff_matrix[1,1,0] = np.array([1, 8, 8])
    payoff_matrix[0,0,1] = np.array([6, 7, 7])
    payoff_matrix[1,0,1] = np.array([7,10,10])
    payoff_matrix[0,1,1] = np.array([3, 8, 8])
    payoff_matrix[1,1,1] = np.array([10,0, 0])

    # 3_player_game_same8
    # payoff_matrix[0,0,0] = np.array([2, 1, 1])
    # payoff_matrix[1,0,0] = np.array([2, 9, 9])
    # payoff_matrix[0,1,0] = np.array([7, 8, 8])
    # payoff_matrix[1,1,0] = np.array([3, 9, 9])
    # payoff_matrix[0,0,1] = np.array([2, 7, 7])
    # payoff_matrix[1,0,1] = np.array([9, 7, 7])
    # payoff_matrix[0,1,1] = np.array([3, 8, 8])
    # payoff_matrix[1,1,1] = np.array([7, 6, 6])

    # 3_player_game_same10_not_satified_constraint_but_mixed_nash
    # payoff_matrix[0,0,0] = np.array([7,10,10])
    # payoff_matrix[1,0,0] = np.array([7, 3, 3])
    # payoff_matrix[0,1,0] = np.array([3, 4, 4])
    # payoff_matrix[1,1,0] = np.array([9, 7, 7])
    # payoff_matrix[0,0,1] = np.array([10,1, 1])
    # payoff_matrix[1,0,1] = np.array([7, 7, 7])
    # payoff_matrix[0,1,1] = np.array([0, 0, 0])
    # payoff_matrix[1,1,1] = np.array([8, 1, 1])

    # 3_player_game_1_Friday case
    # payoff_matrix[0,0,0] = np.array([4, 4,10])
    # payoff_matrix[1,0,0] = np.array([1,10, 1])
    # payoff_matrix[0,1,0] = np.array([0, 8, 5])
    # payoff_matrix[1,1,0] = np.array([1, 6, 5])
    # payoff_matrix[0,0,1] = np.array([2, 4, 6])
    # payoff_matrix[1,0,1] = np.array([9, 3, 0])
    # payoff_matrix[0,1,1] = np.array([8, 4, 9])
    # payoff_matrix[1,1,1] = np.array([0, 0, 8])

    # 3_player_game_5_Friday case
    # payoff_matrix[0,0,0] = np.array([0, 2, 7])
    # payoff_matrix[1,0,0] = np.array([6, 8, 4])
    # payoff_matrix[0,1,0] = np.array([3, 1,10])
    # payoff_matrix[1,1,0] = np.array([10,9, 3])
    # payoff_matrix[0,0,1] = np.array([0,10, 7])
    # payoff_matrix[1,0,1] = np.array([9,10, 1])
    # payoff_matrix[0,1,1] = np.array([3, 7, 7])
    # payoff_matrix[1,1,1] = np.array([8, 5, 6])

    special_game = GeneralSumGame(3, payoff_matrix)
    # for t in [9]:#range(10):
    #     game = GeneralSumGame(3)
    strategy = algo_run_v3(special_game, save_id='3_player_game_same' + str(5))
    # print('Result:\n', strategy)
    # expected_payoff = NashEquilibriumJudger.get_payoff(strategy, game.payoff_matrix)
