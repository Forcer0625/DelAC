...
def get_all_strategy(x):
    ...
    g3 = 1.0 - g0 - g1 - g2
    return np.array([x0, x1, g0, g1, g2, g3])
...
def calculate_expected_payoff(player_idx, game:GeneralSumGame, strategy):
    ...
    Args:
        ...
        game (GeneralSumGame): 為two-player approximation, 其payoff matrix的shape為(2, 4, 2)
        strategy (list): 玩家策略的機率, 長度為6

    Returns:
        float: 指定玩家的期望payoff
    """
    # 分離玩家的策略
    player_0_strategy = strategy[:2]
    player_1_strategy = strategy[2:]
    ...
    # 遍歷所有可能的動作組合
    for action_0 in range(2):  # Player 0 has 2 actions
        for action_1 in range(4):  # Player 1 has 4 actions
            # 計算這個動作組合的聯合機率
...
def feasibility_run(game:GeneralSumGame, n_players=3, tol=1e-8, independent=True) -> np.ndarray:  # 設置容忍值:
    n_players = n_players
    two_player_approximation = game.two_player_approximation(0)
    # 1. 邊界
    bounds = Bounds(0.0, 1.0)
    player_strategy_probs = []
    A = np.ones((1, 1 + 2**(n_players-1) - 1))#np.array([[0.0, 1.0, 1.0, 1.0]]) # g0+g1+g2 <= 1
    A[0, 0] = 0.0
    b = np.array([1.0])
    player_strategy_probs.append(LinearConstraint(A, 0.0, b))

    for i in range(1 + 2**(n_players-1) - 1):
        A = np.zeros((1, 1 + 2**(n_players-1) - 1))#np.array([[1.0, 0.0, 0.0, 0.0]]) # x
        A[0, i] = 1.0
        player_strategy_probs.append(LinearConstraint(A, 0.0, b))
    ...
    def basic_penlity(x):
        ...
        for p in strategys:
            if p < 0.0 or p > 1.0:
                sum += 1
        if not np.isclose(np.sum(strategys[2:]), 1.0):
            sum += 3
        return sum
    ...
    info['u'] = np.array([u1_star, u1_star, u2_star, u2_star, u2_star, u2_star])
    info['v'] = np.array([v1_0, v1_1, v2_0, v2_1, v2_2, v2_3])
    info['r'] = r
    info['indifference'] = indifference
    if independent:
        info['independent'] = all_strategy[2]*all_strategy[5]-all_strategy[3]*all_strategy[4]

    # Record original 2-player approximate game constraint infomation
    p2_p3_strategy = compute_marginal_pmf(all_strategy[2:])
    all_strategy = all_strategy.reshape(3, 2)
    all_strategy[1], all_strategy[2] = p2_p3_strategy[0], p2_p3_strategy[1]
    all_u = NashEquilibriumJudger.get_payoff(all_strategy, game.payoff_matrix)
    _, u2_star, u3_star = all_u[0], all_u[1], all_u[2]
    
    # Virtual Player
    v2_0 = np.concatenate((all_strategy[0], np.array([1.0, 0.0], dtype=np.float64), all_strategy[2])).reshape(3, -1)
    v2_1 = np.concatenate((all_strategy[0], np.array([0.0, 1.0], dtype=np.float64), all_strategy[2])).reshape(3, -1)
    v3_0 = np.concatenate((all_strategy[0], all_strategy[1], np.array([1.0, 0.0], dtype=np.float64))).reshape(3, -1)
    v3_1 = np.concatenate((all_strategy[0], all_strategy[1], np.array([0.0, 1.0], dtype=np.float64))).reshape(3, -1)
    v2_0 = NashEquilibriumJudger.get_payoff(v2_0, game.payoff_matrix)
    v2_0 = v2_0[1]
    v2_1 = NashEquilibriumJudger.get_payoff(v2_1, game.payoff_matrix)
    v2_1 = v2_1[1]
    v3_0 = NashEquilibriumJudger.get_payoff(v3_0, game.payoff_matrix)
    v3_0 = v3_0[2]
    v3_1 = NashEquilibriumJudger.get_payoff(v3_1, game.payoff_matrix)
    v3_1 = v3_1[2]

    r = np.array([u1_star, u1_star, u2_star, u2_star, u3_star, u3_star]) - \
        np.array([v1_0, v1_1, v2_0, v2_1, v3_0, v3_1])

    indifference = r * all_strategy.flatten()

    info['_u'] = np.array([u1_star, u1_star, u2_star, u2_star, u3_star, u3_star])
    info['_v'] = np.array([v1_0, v1_1, v2_0, v2_1, v3_0, v3_1])
    info['_r'] = r
    info['_indifference'] = indifference
